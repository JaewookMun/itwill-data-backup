...
// 일반적으로 테스트 프로그램은 DAO 클래스의 메소드나 Service 클래스의 메소드를 검사하기 위한 목적으로 작성
	// 일반적으로 서비스 프로그램을 테스트하는 경우가 더 많다. - Service클래스가 DAO의 메소드를 사용하기 때문에 둘다 테스트 가능.
@RunWith(SpringJUnit4ClassRunner.class)
// @WebAppConfiguration : WebApplicationContext 객체(Spring Container)를 사용하여 
// Spring Bean을 관리하도록 설정하는 어노테이션  
@WebAppConfiguration
@ContextConfiguration(locations = {"file:src/main/webapp/WEB-INF/spring/root-context.xml"
		,"file:src/main/webapp/WEB-INF/spring/appServlet/servlet-context.xml"})
	// servlet-context의 설정을 통해 annotation을 가지고 bean으로 등록했기 때문에
