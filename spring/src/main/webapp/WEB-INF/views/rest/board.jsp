...
<body>
	...
	function boardDisplay(pageNum){
		...
		$.ajax({
			...
			success: function(json){
				if(json.restBoardList.length==0){
					...
					// JSON을 HTML로 쉽게 바꾸는 방식 - JSON to HTML 많음.
				...
				// 템플릿 객체에 자바스크립트 객체를 전달하여 HTML 코드로 변환하여 반환받아 출력
		...
		// HandleBars는 최근에는 잘 사용되지 않는다. - React로 대체되었다고 보면 된다. (3~4년 전에는 많이 사용)
	...
	// [글쓰기]를 클릭한 경우 호출되는 이벤트 처리 함수를 등록 - 게시글을 입력받아 테이블에 저장
	...
	$("#insertBtn").click(function(){
		...
		$.ajax({
			...
			contentType: "application/json",
				// ajax는 무조건 form data로 전달하기 때문에 위처럼 headers를 설정하거나 contentType을 json 형태로 바꿔줘야한다.
					// -> application/x-www-form-urlencoded 형태의 데이터
			// JSON.stringify(Object object) : 자바스크립트 객체를 JSON 형식의 문자값으로 변경하는 메소드
			// => 요청 처리 메소드의 @RequestBody 어노테이션을 이용한 매개변수에 전달하여 저장
				// 이렇게 해야 하나의 덩어리로 전송가능 - 작성하지 않으면 encoding data로 전송된다.
