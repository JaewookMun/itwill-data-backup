...
@Controller
// @SessionAttributes : 컨트롤러 요청 처리 메소드의 뷰페이지에게만 제공되는 객체를 현재 컨트롤러의 모든
...
// value 속성 : Session Scope로 제공하기 위한 속성명을 속성값으로 설정
// => 다른 속성이 없는 경우 속성값만 설정 가능
	// hewon이라는 이름으로 저장된 객체는 모든 리소스가 사용 가능하다는 의미
@SessionAttributes("hewon")
	// SessionAttributes는 클래스 위에만 줄 수 있음.
	// 정보를 변경할 때 불필요한 전달이나 검색을 줄일 수 있기 때문에 편리하다.
public class SessionController {
	...
	// 아이디를 전달받아 회원정보를 검색하여 뷰페이지에 제공하는 요청처리 메소드
	...
	public String hewonView(Model model) {
		...
		// Model 객체를 이용하여 뷰페이지에 검색 회원정보 제공 - Request Scope
	...
	*/
	...
	public String hewonUpdate(Model model) {
		...
		// hewonView()에서 저장한 속성을 hewonUpdate()에서도 사용 가능.
	
	// 회원정보를 전달받아 회원정보를 변경하는 요청처리 메소드
	// @SessionAttributes 어노테이션에 의해 제공된 객체를 매개변수에 전달하여 저장하고 전달값을 받아 필드값 변경
	...
	public String hewonUpdate(@ModelAttribute Hewon hewon, SessionStatus status) {
		System.out.println("변경아이디 = "+hewon.getId());
