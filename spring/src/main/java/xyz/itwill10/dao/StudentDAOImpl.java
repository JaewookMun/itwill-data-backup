...
// 4. log4j.xml 파일에서 로그 이벤트에 대한 기록 설정
	// mybatis의 명령이 제대로 실행되는지 확인하는 것은 굉장히 중요하다.

// DAO 클래스 : 저장매체에게 행에 대한 삽입,변경,삭제,검색 기능을 제공하는 클래스
// => DBMS 서버에 SQL 명령을 전달하여 실행한 결과를 Java 자료형의 객체로 반환하는 기능의 메소드를 제공
...
// component-scan 엘리먼트를 사용하여 DAO 클래스가 작성된 패키지를 지정
	// transaction 관련 기능 - eg) function
@Repository
public class StudentDAOImpl implements StudentDAO {
	...
	// => 필드마다 어노테이션을 이용하여 인젝션 처리
		// 어노테이션을 사용하면 setter 메소드를 선언할 필요가 없음.
	@Autowired
