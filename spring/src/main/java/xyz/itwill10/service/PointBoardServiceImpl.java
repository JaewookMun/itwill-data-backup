...
public class PointBoardServiceImpl implements PointBoardService {
	...
	// => INSERT, UPDATE, DELETE 명령 관련 DAO 클래스의 메소드를 호출하는 Service 클래스의 메소드에 설정
		// Transactional 어노테이션을 사용하는 것이 더 간편
		// mybatis에서만 @Transactional을 사용하는 것이 아니라 JPA, hibernate, jdbc 등 다른 db 작업에서도 사용할 수 있다.
	@Transactional
	@Override
	public PointUser addPointBoard(PointBoard board) {
		...
		if(pointUserDAO.selectPointUser(board.getWriter())==null) {
			throw new RuntimeException("게시글 작성자가 존재하지 않습니다.");
				// Service 클래스의 메소드에서 예외가 발생하면 처리된 명령들은 취소될 필요가 있다.(RollBack)
	...
	// => 게시글 작성자를 전달하여 POINTUSER 테이블에 저장된 사용자의 포인트가 감소되도록 작성
		// Service 클래스의 메소드는 DAO 클래스의 메소드를 다양하게 호출하여 원하는 기능을 구현
	@Transactional
	@Override
	public PointUser erasePoinBoard(int num) {
	...
	@Override
	public List<PointBoard> getPointBoadrList() {
