...
// => renderMergedOutputModel() 메소드를 오버라이드 선언하여 응답 처리시 실행될 명령을 작성
	// 어노테이션을 사용해도 되자만 관리측면에서 bean element로 등록 (comp:scan을 또 작성해줘야함.)
public class FileDownload extends AbstractView {
	...
	// => Map 자료형의 매개변수에 요청 처리 메소드에서 Model 객체에 의해 제공되는 정보 저장
	@Override
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		// 클라이언트에게 파일을 전달하기 위한 응답정보 변경
			// 위에서 가져온 contentType을 가져와 전달 - 다운로드
		response.setContentType(getContentType());
		response.setContentLength((int)downloadFile.length());
		
		// 클라이언트에게 다운로드 파일명 전달
		originFilename=URLEncoder.encode(originFilename, "utf-8");
			// 브라우저마다 하는 방식이 조금씩 다르다.
			// 최신버전은 이렇게 해도 되지만 원래는 if문을 사용하여 브라우저 조건을 확인하여 설정
		...
		try {
			// 다운로드 파일을 원시데이터로 읽기 위한 입력스트림을 생성하여 생성
			in=new FileInputStream(downloadFile);
			
			// FileCopyUtils.copy(InputStream in, OutputStream out) : 입력스트림을 이용하여
			// 원시데이터를 읽어 출력스트림으로 반복해서 전달하는 메소드 - 파일 복사
