...
public class CreateBeanApp {
	public static void main(String[] args) {
		...
		BeanFactory factory=new XmlBeanFactory(new FileSystemResource("src/main/resources/04-1_beanCreate.xml"));
			// 이제 사용되지 않기 때문에 deprecated 경고가 발생
		System.out.println("========== Spring Container 초기화 후 =======");
		CreateBean bean2=(CreateBean)factory.getBean("createBean");
		bean2.display();
		System.out.println("===================================================================");
		
			/* Spring Container 역할을 하는 클래스
				BeanFactory (old) / ApplicationContext (latest)를 상속받은 클래스 사용
				=> ApplicationContext 상속 클래스 사용! 
				AC -> 미리 모든 객체 생성 / BF -> 사용할 때 객체 생성
				ApplicationContext를 통해 Spring Bean으로 등록된 클래스에 더 많은 기능을 제공할 수 있기 때문에
				AC 사용권장 (Spring 2.5 버전 떄 ApplicationContext 출시)
