...
public class MyMemberXMLDAO {
	...
	public static MyMemberXMLDAO getDAO() {
		...
		// mybatis 환경설정 파일을 읽어들이는 입력스트림을 생성하여 
		// sqlsessionfactorybuilder로 sqlsession 생성하여 반환
	...
	public int insertMember(MyMember member) {
		...
		try {
			...
			// SqlSession.commit() : 트랜젝션 적용 명령(COMMIT)을 전달하여 실행하는 메소드
			// SqlSession.rollback() : 트랜젝션 취소 명령(ROLLBACK)을 전달하여 실행하는 메소드
	...
	public int deleteMember(String id) {
		...
		try {
			...
			// 엘리먼트의 SQL 명령을 제공받아 실행 후 삽입행의 갯수를 반환하는 메소드
			return sqlSession.update("xyz.itwill.mapper.MyMemberXMLMapper.deleteMember", id);
		} finally {
			sqlSession.close();
		}
	}
