...
//=> 프로그램에 인스턴스를 하나만 제공하는 클래스 - 싱글톤 클래스 (대표적인 클래스임)
	//JVM은 하나밖에 존재하지 않음. 그렇기 때문에 인스턴스를 여러개 만드는 것은 비효율적이다.
public class RuntimeApp {
	public static void main(String[] args) {
		...
		//Runtime.totalMemory() : JVM이 사용할 수 있는 전체 메모리 크기를 반환하는 메소드
		//Runtime.freeMemory() : JVM 프로그램이 사용할 수 있는 남은 메모리 크기를 반환하는 메소드
		
		System.out.println("청소 전 사용 메모리 크기 = "+(Runtime.getRuntime().totalMemory()-Runtime.getRuntime().freeMemory())+"Byte");
			//가비지컬랙터는 JVM에서 불필요한 객체를 삭제하는 역할을 함
		//Runtime.gc() : Garbage Collector 프로그램을 실행하는 메소드
		//=> System.gc() 메소드 호출과 동일한 기능 제공 
		//Runtime.getRuntime().gc();
		System.gc();
		System.out.println("청소 후 사용 메모리 크기 = "+(Runtime.getRuntime().totalMemory()-Runtime.getRuntime().freeMemory())+"Byte");
		System.out.println("===========================================================");
	}
}
