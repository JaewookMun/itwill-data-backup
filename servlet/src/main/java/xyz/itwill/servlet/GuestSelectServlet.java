...
// => [방명록 쓰기]를 클릭하면 방명록  게시글 입력페이지 (insert.html)를 요청하여 페이지 이동
// => 방명록 게시글의 [삭제]를 클릭하면 방명록 게시글 삭제 처리페이지(delete.itwill)를 요청하여 페이지 이동 - 방명록 게시글번호 전달
...
public class GuestSelectServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("</body>");
		out.println("</html>");
			// Servlet의 단점 : HTML 문서를 만드는 것이 어렵다.
			// -> JSP 가 나온 이유. HTML처럼 웹문서의 역할을 하지만 프로그램의 역할도 한다.
			// data처리는 Servlet으로 하고 응답은 JSP로 한다고 보면된다.
	}
}
