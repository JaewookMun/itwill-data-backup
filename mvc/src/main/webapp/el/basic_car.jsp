...
<body>
	...
	<%-- => 필드명은 Getter 메소드를 호출하기 위한 이름 --%>
		<%-- getAttribute를 사용하지 않아도 속성값을 가져올 수 있고
		getter 메소드를 사용하지 않아도 getter 메소드가 자동 호출되는 기능을한다. 
		'get'이 생략되었다고 생각가능.
		--%>
	<p>자동차 모델명 = ${car.modelName }</p>
	<%-- EL 표현식의 필드명에 대한 Getter 메소드가 없는 경우 PropertyNotFoundException 발생 --%>
	<%-- <p>자동차 모델명 = ${car.name }</p> --%>
		<%-- getter 메소드를 못찾아서 에러가 발생하는 경우를 제외하고 에러가 발생할 일이 없다. --%>	
	<p>자동차 색상=${car.carColor }</p>
	<%-- 속성명의 속성값이 없는 경우 EL 미실행 --%>
	<p>자동차 모델명 = ${pageCar.modelName }</p>
		<%-- getAttribute를 사용하면 NullPointerException이 발생하지만 여기서는 발생할 일이 없음. --%>
</body>
</html>