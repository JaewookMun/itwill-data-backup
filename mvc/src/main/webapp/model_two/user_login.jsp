...
<%-- 세션에 저장된 권한 관련 정보를 이용하여 로그인 사용자와 비로그인 사용자를 구분하여
다른 응답결과를 클라이언트에게 전달하는 JSP 문서 --%>
<%-- 비로그인 사용자 : 인증정보를 입력받기 위한 기능 제공 --%>
<%-- [로그인]을 클릭한 경우 로그인 처리페이지(login.do)로 이동 - 인증정보 전달 --%>
<%-- 로그인 사용자 : 환영메세지를 전달하는 기능 제공 --%>
<%-- => [회원목록]을 클릭한 경우 회원목록 출력페이지(list.do)로 이동 --%>
<%-- => [회원등록]을 클릭한 경우 회원정보 입력페이지(logout.do)로 이동 - 관리자만 가능 - --%>
<%-- => [로그아웃]을 클릭한 경우 로그아웃 처리페이지(writeForm.do)로 이동 --%>

	<%-- 리소스의 요청 및 응답은 Controller를 기준으로 판단 --%>
...
<%-- 요청 URL 주소의 경로와 응답 JSP 문서의 경로가 같지 않으므로 리소스 파일 경로는
절대 경로로 표현하는 것을 권장 --%>
