...
<body>
	...
	<hr>
	<%-- 서버에 저장된 리소스 파일(CSS파일, JS 파일, 멀티미디어 파일 등)을 상대경로로 표현하여 제공 --%>
	<%-- 상대경로 : 현재 요청 웹프로그램의 경로를 기준으로 리소스 위치 표현 --%>
	...
	<%-- 절대경로 : 루트 디렉토리(서버 디렉토리)를 기준으로 리소스 위치 표현 --%>
		<%-- 절대경로의 문제 - context 경로가 바뀔 수 있는점... --%>
		<%-- 컨텍스트 경로에 영향을 주지 않도록 만드는 것이 중요하다. --%>
	...
	<%--
		EL표현식 or url 태그 둘중 하나를 이용하여 경로표현  
	--%>
