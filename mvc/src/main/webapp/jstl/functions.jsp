...
<%-- Functions 태그 라이브러리를 JSP 문서에 포함 - prefix 속성값은 "fn"으로 설정 --%>
...
<body>
	...
	<p>전화번호 = ${phone }</p>
	<hr>
		<%-- Functions 함수는 String 클래스의 메소드들을 생각하면된다. --%>
	...
	<div>${fn:replace(content,'\\n','<br>') }</div>
		<%-- <div>${fn:replace(content,'\n','<br>') }</div> --%>
		<%-- 사용불가능 \n에서 '\'은 특수문자이기 때문에 escape 될 수 없음. --%>		
		<%-- 앞에 역슬래시(\)를 하나 더 표기해서 escape 처리해야함. --%>
	<hr>
	<c:set var="html" value="<font size='7' color='red'>안녕하세요.</font>"/>
	<%-- HTML 태그가 포함된 EL를 이용하여 출력할 경우 HTML 태그로 적용되어 출력 --%>
	<div>${html }</div>
