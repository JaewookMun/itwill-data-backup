...
<body>
	...
	<%-- value 속성 : 스코프 객체에 저장된 속성값(숫자값)을 속성값으로 설정 --%>
	<%-- type 속성 : number(숫자) 또는 currency(화폐) 중 하나를 속성값으로 설정 --%>
	<%-- => 속성값인 [number]인 경우 숫자 3자리마다 ,가 삽입된 형식의 문자열로 변환되어 제공 --%>
	<p>가격 = <fmt:formatNumber value="${price }" type="number"/>원</p>
	<%-- => 속성값인 [currency]인 경우 앞에 화폐기호가 삽입되고 숫자 3자리마다 ,가 삽입된 
	형식의 문자열로 변환되어 제공 --%>
	<p>가격 = <fmt:formatNumber value="${price }" type="currency"/></p>
	
	<%-- pattern 속성 : 숫자를 표현하는 패턴문자를 속성값으로 설정 --%>
		<%-- 잘 사용하지 않음. --%>
	<p>가격 = <fmt:formatNumber value="${price }" pattern="$##,###,###,##0.00"></fmt:formatNumber>
</body>
</html>