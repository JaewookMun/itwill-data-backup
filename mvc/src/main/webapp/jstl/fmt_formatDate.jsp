...
<body>
	...
	<c:set var="now" value="<%=new Date()%>"/>
		<%-- 원칙적으로는 모델에서 인스턴스를 만들어서 제공 여기서는 모델이 없기 때문에 임시적으로 위처럼 표기 --%>
	<p>now = ${now }</p>
	
	<%-- formatDate 태그 : 스코프 객체에 저장도니 Date 인스턴스의 날짜와 시간을 원하는 패턴의
	...
	<p>now(날짜)	= <fmt:formatDate value="${now }" type="time"/>
	...
	<%-- => 속성값이 [short]인 경우 "a h:mm" 패턴의 문자열로 변환되어 제공 --%>
		<%-- full에서 z는 time zone을 의미 --%>
	<p>now(날짜)	= <fmt:formatDate value="${now }" type="time" timeStyle="full"/> 	
	<p>now(날짜)	= <fmt:formatDate value="${now }" type="time" timeStyle="short"/> 	
