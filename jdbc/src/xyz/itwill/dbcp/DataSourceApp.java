...
//UCP(Universal Connection Pool) 라이브러리의 DBCP 관련 클래스를 이용한 JDBC 프로그램
	//향후 Apache나 Spring의 DBCP 사용할 것 하지만 기본 골자는 같음. - DataSource 상속
public class DataSourceApp {
	public static void main(String[] args) throws SQLException {
		//PoolDataSource : DBCP 기능을 제공하기 위한 인스턴스
		//PoolDataSourceFactory.getPoolDataSource() : PoolDataSource 인스턴스를 생성하여 반환 
			//Factory 클래스는 대부분 싱글톤 디자인패턴으로 생성 - 인스턴스를 만들어서 반환하는 클래스
			//Factory 클래스가 있으면 인스턴스를 반환하는 메소드를 사용.
			// Factory 클래스 - 특정 클래스의 인스턴스를 만들고 이를 관리하기 위한 클래스(?)
			// Factory관련 Design Pattern 확인
		...
		//PoolDataSource 인스턴스에 미리 생성되어 저장될 Connection 인스턴스의 제한갯수를
		...
		//Connection 인스턴스를 생성하여 추가 - Connection 갯수 유지
		con1.close();
