...
//=> ResultSet 커서의 위치 변경 또는 ResultSet 커서 위치의 처리행에 대한 컬럼값 조작 가능
	//ResultSet 커서를 원하는 행으로 이동시킬 수 있다.

//STUDENT 테이블에 저장된 모든 학생정보를 검색하여 출력하는 JDBC 프로그램 작성
	//특별한 프로그램을 만드는 것이 아닌이상 크게 사용할 일이 없다.
public class ResultSetCursorApp {
	public static void main(String[] args) throws SQLException {
		...
		while(rs.next()) {
			...
			System.out.println(rs.getRow()+"행 : 학번 = "+rs.getString("no") + ", 이름 = "+rs.getString("name"));
		}
		...
		//=> 매개변수에 전달되는 값에 따라 SELECT 명령의 실행결과를 저장한 ResultSet 인스턴스의 속성변경
		...
		//Result.previous() : ResultSet 커서를 이전행으로 이동하는 메소드
		while(rs.previous()) {
			System.out.println(rs.getRow()+"행 : 학번 = "+rs.getString("no") + ", 이름 = "+rs.getString("name"));
		}
		...
		//=> 삽입된 행 다음에 존재하는 기존행은 자동으로 다음행으로 이동
		rs.moveToInsertRow();
		...
		//rs.updateString("birthday", new Date(100,11,20));//2000-12-20
		rs.updateString("birthday", "2000-12-20 00:00:00.0");
		
		//ResultSet.insertRow() : 커서 위치의 행으로 테이블에 삽입하는 메소드
		rs.insertRow();
		*/
		...
		//ResultSet.delete() : 커서 위치의 행으로 테이블의 행을 삭제하는 메소드
		rs.deleteRow();
		*/
		
		// ResultSet 인스턴스의 2번째 위치의 행에 이름을 [임걱정]으로 변경하여 테이블에
		rs.absolute(2);
		rs.updateString("name", "임걱정");
		
		//ResultSet.update() : 커서위치의 행으로 테이블의 행을 변경하는 메소드 
		rs.updateRow();
