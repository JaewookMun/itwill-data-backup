...
//실행되면 자동으로 커밋 처리
//=> 프로그램 실행시 예외가 발생된 경우 예외 발생전에 실행된 SQL 명령에 대한 롤백처리가 불가능
//AutoCommit 기능을 비활성화 처리하여 프로그램이 정상적으로 실행된 경우 커밋 처리하고
//예외가 발생된 경우 롤백 처리하는 것을 권장
	//TRANSACTION : DML명령을 세션에 저장하기 위한 논리적인 작업공간
	//executeUpdate() 메소드를 사용한 순간 명령은 처리되고 자동커밋된다.
...
public class TransactionControlApp {
	public static void main(String[] args) {
		...
		try {
			...
			//인위적 예외 발생
				//여기서는 에러가 발생하지 않기 때문에 예시를 위해 생성
				//정상적이면 commit, 비정상적이면 rollback을 하는 프로그램 작성
				//단순히 sql 명령을 하나만 전달할 때는 이런 형태를 사용하지 않음. 
				//=> sql명령을 여러번 실행해야 하는 프로그램 제작 시 사용
				//Spring 때 다시 공부
			...
			System.out.println("[에러] JDBC 에러 = "+e.getMessage());
		} catch (Exception e) {
