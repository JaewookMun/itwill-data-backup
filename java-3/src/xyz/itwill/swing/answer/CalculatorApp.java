package xyz.itwill.swing.answer;
...
import javax.swing.border.EmptyBorder;
		...
		//label이나 button을 필드로 선언해야 디자인핸들러로 처리할 수 있다.
public class CalculatorApp extends JFrame implements ActionListener {
	...
	public CalculatorApp() {
		...
		b0.addActionListener(this);	//Frame 클래스가 action listener를 구현
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(eventButton==bEquals) {//"=" 버튼을 누른 경우
			...
			for(int i=0; i<operatorArr.length; i++) {
				index=operation.lastIndexOf(operatorArr[i]);
				if(index!=-1) break;
			}
			...
			try {
				...
				label.setText(result+"");
					...
					//try문이 수행되면 무조건 예외가 발생되게 만드는 코드  
				//throw new Exception();
				
				//연산식 저장 필드 초기화
				//operation="";
				operation=result+"";//연산식의 결과값을 연산식으로 계속 사용
					//0으로 나눴을 때 ArithmeticException 발생
				...
				JOptionPane.showMessageDialog(this, "프로그램에 예기치 못한 오류가 발생되었습니다. 프로그램을 종료합니다."
						, "에러", JOptionPane.ERROR_MESSAGE);
				System.exit(0);
			}
					//cf. 연산자를 한번 누르면 나머지 연산자를 비활성화 처리할 수도 있다. - boolean 변수.
			
		} else if(eventButton==bClear) {//"C" 버튼을 누른 경우
			operation="";//연산식 저장필드 초기화
			label.setText("0");
			...
			//JComponent.setText() : Swing 컴퍼넌트의 문자열을 변경하는 메소드
