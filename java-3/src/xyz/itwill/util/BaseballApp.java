...
//=> 난수와 입력값은 3개의 정수값으로 서로 다르며 0으로 시작 불가능
//=> 난수와 입력값을 비교하여 동일한 정수값이 존재할 때 위치가 같으면 스트라이크, 다르면 볼로 계산
//=> 스트라이크가 3개면 성공메세지 출력 후 프로그램을 종료하고 아니면 스트라이크와 볼의 갯수 출력
...
public class BaseballApp {
	public static void main(String[] args) {
		...
		for(int cnt=1; cnt<=15; cnt++) {
			//스트라이크와 볼의 갯수를 저장하기위한 변수 선언
			int strike=0, ball=0;
			//0~9 범위의 입력값 3개를 저장하기 위한 배열 선언
			int[] num = new int[3];
			
			//규칙에 맞는 입력값이 배열 요소에 저장되도록 처리하는 반복문
			loop:
			while(true) {
			...
			for(int i=0; i<dap.length; i++) {//난수값에 대한 배열 요소의 첨자
				for(int j=0; j<num.length; j++) {//입력값에 대한 배열 요소의 첨자
					if(dap[i]==num[j]) {//난수 배열의 요소값과 입력 배열의 요소값이 같은 경우
