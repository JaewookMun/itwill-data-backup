...
//외부 클래스(Outer Class) : 내부 클래스가 선언된 클래스
	//컴파일하면 바이트코드파일은 두개가 된다.
	//Outer.class 와 Outer$inner.class로 만들어짐 >> $가 도트연산자 역할을 함
public class Outer {
	...
	public void outerInnerDisplay() {
		...
		//외부 클래스 메소드에서 내부 클래스로 인스턴스 생성 가능
		Inner inner = new Inner();
		//내부 클래스로 생성된 인스턴스를 이용하여 필드 또는 메소드 접근 가능
		//=> 접근 지정자에 상관없이 필드 또는 메소드 접근
		inner.innerNum=200;
		inner.innerDisplay();
	}
	...
	//=> 외부 클래스에서만 인스턴스를 생성하여 사용할 수 있는 클래스
	public class Inner {
		...
		//내부 클래스의 메소드에서는 외부 클래스의 필드 또는 메소드 접근 가능
			//외부클래스는 내부 클래스의 인스턴스를 만들어 내부클래스에 접근 가능하지만
			//내부클래스는 외부클래스에 직접적인 접근이 가능하다. 
			// : >> 접근자 상관없음. (private 필드에 접근 가능)
	...
	//=> 내부 클래스의 메소드에서는 외부 클래스의 필드 또는 메소드 접근 불가능
	//=> 내부 클래스의 메소드에서는 외부 클래스의 정적 필드 또는 정적 메소드 접근 가능
	//=> 외부 클래스와 상관없이 인스턴스를 생성하여 사용 가능한 클래스
