...
public class NotepadApp extends JFrame implements ActionListener {
		//notepadApp으로 객체입출력을 할 것이 아니기 때문에 UID를 default로 설정
	private static final long serialVersionUID = 1L;
	
		//이벤트 핸들러에서 사용하기 위해 필드로 생성(이벤트 핸들러를 이용하지 않을거면 여기에 선언할 필요 x)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(eventSource == init) {
			...
			filePath = openDialog.getDirectory()+openDialog.getFile();
				...
				//override된 메소드에서 예외는 못떠넘긴다.(Throws 사용 불가능)
			try {
				//파일 입력 스트림을 생성하여 저장
				...
				//=> 파일 내용을 한 줄씩 읽어 JTextArea 컴퍼넌트에 추가 - 반복 처리
					// 엔터('\n') - 실제 데이터를 구분하는 구분자이다.
				while(true) {
					//파일 입력 스트림으로 엔터(Enter) 전까지의 문자 데이터를 읽어 문자열로 반환
				...
				JOptionPane.showMessageDialog(this, "선택한 파일이 없습니다.","에러",JOptionPane.ERROR_MESSAGE);
			} catch (IOException exception) {
				JOptionPane.showMessageDialog(this, "프로그램에 문제가 발생 하였습니다.","에러",JOptionPane.ERROR_MESSAGE);
			}
			...
			try {
				...
				//JTextArea 컴퍼넌트의 모든 문자열을 반환받아 저장
					//한줄씩 읽어들이는 것이 아니기 때문에 반복처리하지 않는다.
					//통째로 읽어들이고 통째로 저장한다.
	...
	public boolean saveConfirm() { //JTextArea 컴퍼넌트의 내용이 변경된 경우
		//JOptionPane.showConfirmDialog(Component parent, Object Message
		//, String title, int option) : 선택 다이얼로그를 출력하는 메소드
		//=> 선택값(정수값) 반환
