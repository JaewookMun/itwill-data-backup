<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- xml_two.jsp 문서를 AJAX 기능으로 요청하여 뉴스목록을 XML 형식의 텍스트 데이터로 
응답받아 클라이언트에게 전달하는 JSP 문서 --%>
<%-- XML : 엘리먼트(태그)를 이용하여 값(정보)을 구분하는 구조적인 데이터 표현 방법 --%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
	...
	function displayNewsList(){
		if(xhr.readyState==4){
			// XML 텍스트 데이터로 응답받은 결과는 XMLHttpRequest 객체의 responseXML 속성을
			// 이용하여 XMLDocument 객체로 반환받아 저장
			if(xhr.status==200){
					// parsing 을 통해 보기 쉽게 해줘야함
				...
				for(i=0; i<newsList.length; i++){
					...
					// Element 객체(news 엘리먼트)의 자식 엘리먼트의 노드값을 제공받아 저장 
				...
				document.getElementById("newsContent").innerHTML=html;
					// parsing 방식이 다르다. - xml > DOM parsing (DOM parser)
			} else {
