...
<body>
	...
	<div>
		...
		<%-- 동기식 통신일 때는 번역하기 버튼 결과를 완전히 받기전까지 '다음'을 이용할 수 없다. --%>
	
	<script type="text/javascript">
		// 비동기식 통신을 위한 XMLHttpRequest 객체(Ajax Engine)를 저장하기 위한 전역변수
			// Ajax 엔진이 있어야 비동기식 통신을 진행가능하며 모든 함수에서 사용하기 위해 전역변수로 선언한다.
		var xhr=null;
		
		// XMLHttpRequest 객체를 생성하여 반환하는 함수 선언
		// => 브라우저의 종류와 버전에 따라 XMLHttpRequest 객체를 생성하거나 사용하는 방법이 다양하므로 함수 선언
		function getXMLHttpRequest(){
				// window(브라우저)에 ActiveXObject 프로퍼티가 있다면...
			if(window.ActiveXObject){ // IE4 ~ IE6
				...
				return new XMLHttpRequest();
					// 현재 대부분의 브라우저는 new XMLHttpRequest();만 사용해도 큰 문제 없다.
			} else {
				return null;
			}
		}
		...
		document.getElementById("btn").onclick=function(){
			...
			// => 다른 웹프로그램을 비동기식으로 요청하여 실행 결과를 응답받아 요소를 변경
			...
			// xhr.open("GET", "hello_two.jsp", false);
				// 동기식으로 설정하면 hello_two.jsp의 결과를 받기전까지 아무런 동작도 할 수 없다.
				// 비동기식 통신은 hello_two.jsp의 결과를 기다리는 동안 다른 작업을 수행할 수 있다.
			...
			xhr.send(null); // 웹프로그램 요청 후 준비상태는 자동으로 [3]과 [4]로 변경
				// 전달값이 없다는 의미 - GET방식은 query string 이용
		...
		function changeHTML(){
			...
			*/
			...
			if(xhr.readyState==4){
				...
				document.getElementById("display").innerHTML="<img src='images/loading.gif' width='10'>";
					// 결과를 느리게 줄것 같으면 loading 이미지를 사용할 수 있다.
			}
		}
	</script>
</body>
</html>
<%-- JS를 통해 document 문서 요소를 객체로 받아 처리 --%>