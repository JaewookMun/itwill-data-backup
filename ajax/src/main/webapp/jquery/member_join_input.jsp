	...
	$("#joinForm").submit(function() {
		...
		if(id=="") {
			...
			$.ajax({
				...
				/*
					input 페이지에서 submit 이벤트가 발생하면 action 페이지를 요청하여 처리
					member_id_check.jsp의 결과를 응답받기 전에 action 페이지로 이동해버린다.
					-> asyc 속성을 false로 설정(동기화)하면 위 문제를 해결가능
					동기식처리를 하면 응답결과를 받을 때까지 멈춰있는다.
					비동기식 통신을하면 아이디 중복검사 결과를 받기전에 submit 되버리는 문제가 있음.
					
					ajax - asyncronous 방식으로 프로그램의 요청과 응답을 주고받는 통신방식
				*/ 
			});
		}
