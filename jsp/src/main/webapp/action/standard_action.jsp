...
<body>
	...
	- 표준 액션 태그는 [jsp] 네임스페이스를 이용하여 사용</p>
		<%-- HTML태그와 구분하기 위해 jsp 네임스페이스(접두사)를 붙임 --%>
	<hr>
	...
	(Getter 메소드 호출) -useBean 태그의 종속 태그</p>
		...
		<%--
			include와 forward 둘다 스레드를 이동시키는 것은 같다.
			include directive가 다른 문서의 코드를 포함시킨다면
			include action 태그는 다른 문서의 실행결과(HTML)를 포함시킨다.
			
			forward는 JSP 문서의 목적을 처리 or 응답의 단일 목적으로 사용
			응답 / 처리 따로 진행 (고급기술에서 처리만 따로 진행하는데 이는 - Servlet으로... JSP ver2에서 ...)
			
			param태그는 include나 forward처럼 독립적으로 사용하지 못하고 자식태그로만 사용가능
			스레드 이동은 두 태그로 가능하기 때문
			
			Bean은 자바 객체를 의미
			set/get 역할을 한다. 없으면 setAttribute/ 있으면 getAttribute
			
			set/getProperty는 useBean 태그의 종속태그이다.!
		--%>
</body>
</html>