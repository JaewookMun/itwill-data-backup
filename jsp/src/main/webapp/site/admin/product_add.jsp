...
<%-- => [제품등록]을 클릭한 경우 제품정보 등록 처리페이지(product_add_action.jsp)로 이동 - 입력값(제품정보) 전달 --%>
...
<div id="product">
	...
	<%-- 파일을 입력받아 전달하기 위해 method 속성값을 [post]로 설정하고
	enctype 속성값을 [multipart/form-data] --%>
	<form action="<%= request.getContextPath() %>/site/index.jsp?workgroup=admin&work=product_add_action"
		...
		<table>
			...
			<tr>
				<td>제품명</td>
				<td>
					<input type="text" name="productName" id="productName" maxlength="20" 
						<% if(product!=null){ %> value="<%= product.getProductName() %>" <% } %>>
				</td>
			</tr>
			<tr>
				<td>제품이미지</td>
			...
			<tr>
				...
				<td>
					<input type="text" name="productQty" id="productQty"
						<% if(product!=null){ %> value="<%= product.getProductQty() %>" <% } %>>
				</td>
			</tr>
			<tr>
				<td>제품가격</td>
				<td>
					<input type="text" name="productPrice" id="productPrice"
						<% if(product!=null){ %> value="<%= product.getProductPrice() %>" <% } %>>
				</td>
			</tr>
			<tr>
				<td colspan="2"><button type="submit">제품등록</button>
			</tr>
		</table>
	</form>
...
<script type="text/javascript">
	$("#productForm").submit(function(){
		...
		if($("#productDetail").val()==""){
			$("#message").text("제품 상세설명을 입력해 주세요.");
			$("#productDetail").focus();
			return false;
		}
		if($("#productQty").val()==""){
			$("#message").text("제품 수량을 입력해 주세요.");
			$("#productQty").focus();
			return false;
		}
		if($("#productPrice").val()==""){
			$("#message").text("제품 가격을 입력해 주세요.");
			$("#productPrice").focus();
			return false;
		}
	});
			// 수량, 가격은 validation 검사를 하는 것이 좋음.
			// 제품코드 중복체크 및 형식에대한 검사 필요.
			// 여기에 버튼을 추가하여 중복검사를 할 수도 있지만 add_action에서 
			// insert하기 전 중복여부를 검사할 수도 있다.
