...
<%-- => [카테고리]를 변경한 경우 제품정보 출력페이지(product_manager.jsp)로 이동 - 카테고리 전달 --%>
	...
	<table>
		...
		<% } else { %>
			...
			<tr>
				...
				<td>
					<a href="<%= request.getContextPath() %>/site/index.jsp?workgroup=admin&work=product_detail&productNum=<%= product.getProductNum()%>">
				...
				<td><%= DecimalFormat.getCurrencyInstance().format(product.getProductPrice()) %></td>
					<%-- DecimalFormat 클래스를 이용하여 숫자를 원하는 형태의 문자열로 표기 --%>
			</tr>
			<% } %>
		<% } %>
	</table>
