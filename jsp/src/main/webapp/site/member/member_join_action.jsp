...
<%
	...
	if(request.getMethod().equals("GET")){
		...
		// => 자바스크립트를 이용하여 페이지 이동
			// 리다이렉트 이동을 하라고 템플릿 페이지에 줘도 클라이언트에게 전달 불가능 sendError, sendRedirect 불가능
			// 템플릿 페이지의 Content 영역에 포함되는 JSP 문서는 리다이렉트 이동, 에러코드 전달이 불가능.
			// -> index.jsp를 기준으로 만들어야한다.
	...
	// => 템플릿페이지(index.jsp)에서 POST 방식으로 전달된 값에 대한 캐릭터셋 변경
		// 아래처럼 표기해도 아무런 효과가 없다. (표기해도 문제는 없다.)
	...
	String address2=Utility.stripTap(request.getParameter("address2"));
	...
	// 회원정보를 전달받아 MEMBER 테이블에 삽입하여 저장하는 DAO 클래스의 메소드 호출
