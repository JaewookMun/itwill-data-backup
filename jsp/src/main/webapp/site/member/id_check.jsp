...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 아이디를 전달받아 MEMBER 테이블에 저장된 회원정보의 아이디와 비교하여 사용 가능 여부를
클라이언트에게 전달하는 JSP 문서 --%>
<%-- => 아이디 사용 가능 : 사용 가능 메세지 전달 - 부모창의 아이디 입력태그의 입력값으로 변경 --%>
<%-- => 아이디 사용 불가능 : 사용 불가능 메세지 전달 - 아이디 입력 받아 현재 페이지 재요청 --%>
<% 
	// 비정상적인 요청에 대한 처리
	...
	// 아이디를 전달받아 MEMBER 테이블에 저장된 회원정보를 검색하여 반환하느느 DAO 클래스의 메소드 호출
...
<body>
	...
	<% } else { // 아이디가 사용 불가능한 경우 %>
		<div id="message">입력한 <span class="id">[<%= id %>]</span>는 이미 사용 중인 아이디 입니다.<br>
		다른 아이디를 입력하고 [확인]을 눌러주세요.</div>	
		<div>
			<%-- form 태그의 action 속성이 생략된 경우 현재 브라우저의 URL 주소 재요청 --%>
