...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 글번호를 전달받아 BOARD 테이블에 저장된 해당 글번호의 게시글을 검색하여 클라이언트
에게 전달하는 JSP 문서 --%>
<%-- => [글삭제]를 클릭한 경우 게시글 삭제 처리페이지(board_remove_action.jsp)로 이동 - 글번호 전달 --%>
<%-- => [글변경]를 클릭한 경우 게시글 변경 입력페이지(board_modify.jsp)로 이동 - 글번호 전달 --%>
<%-- => [답글쓰기]를 클릭한 경우 게시글 입력페이지(board_write.jsp)로 이동 - 답글 관련 정보 전달 --%>
<%-- => [글목록]을 클릭한 경우 게시글 목록 출력페이지(board_list.jsp)로 이동 - 검색 관련 정보 전달 --%>
<% 
	// 비정상적인 요청에 대한 처리
	...
	if(board.getStatus()==1){
		// 비로그인 사용자이거나 로그인 사용자가 작성자 또는 관리자가 아닌 경우 - 비정상적인 접근
	...
	// 게시글번호를 전달받아 BOARD 테이블에 저장된 해당 글번호의 게시글 조회수를 증가하는
	// DAO 클래스의 메소드 호출
...
<div id="board_detail">
	...
	<table>
		<tr>
			...
			<td>
				<%=board.getWriter()%>
				<% if(loginMember != null && loginMember.getStatus()==9) { // 관리자인 경우 IP 조회 %>
					[<%= board.getIp() %>]
				<% } %>
			</td>
		</tr>
	...
	<div id="board_menu">
		...
		<button type="button" id="listBtn">글목록</button>
			<%-- 세션(session)에 저장된 권한정보에 따라 보여지는 화면을 다르게 구성 --%>
	</div>
