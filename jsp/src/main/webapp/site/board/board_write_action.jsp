...
<%
	...
	// => 이클립스에 의해 동작되는 WAS는 IPv6의 128Bit 형식의 IP 주소 제공
	// IPv4의 32Bit 형식의 IP주소를 제공받을 수 있도록 이클립스 환경 설정
	...
	if(ref==0){ // 새글 - ref, reStep, reLevel 변수에 전달값을 반환받아 [0] 저장
		// 새글은 REF 컬럼에 자동 증가값을 저장 - NUM 컬럼에 저장되는 값과 동일
		// => RE_STEP 컬럼과 RE_LEVEL 컬럼에는 [0]을 저장
		ref=num;
	} else { // 답글 - ref, reStep, reLevel 변수에 전달값으로 부모글의 전달값이 저장
		// 부모글의 전달값(ref, reStep, reLevel 변수값)을 이용하여 기존 게시글의 정보를 변경하는
		// DAO 클래스의 메소드 호출
		// => BOARD 테이블에 저장된 게시글 중 부모글에게 전달된 ref 변수값과 REF 컬럼값이
		...
		// 답글의 RE_STEP 컬럼과 RE_LEVEL 컬럼에는 부모글의 전달값이 저장된 reStep 변수와
	...
	board.setRef(ref); // 새글 : 자동 증가값, 답글 : 부모글 + 1
	board.setReStep(reStep); // 새글 : 자동 증가값, 답글 : 부모글 + 1
	board.setReLevel(reLevel); // 새글 : 자동 증가값, 답글 : 부모글 + 1
	...
	// 게시글 목록 출력페이지 이동 - 페이지 번호 전달(새글 : 1, 답글 : 부모글의 전달값)
	out.println("<script type='text/javascript'>");
