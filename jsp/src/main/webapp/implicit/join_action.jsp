...
<%
	...
	if(request.getMethod().equals("GET")){ // JSP 문서를 GET 방식으로 요청한 경우
		...
		// => 리다이렉트 이동은 ServerRoot를 최상위 디렉토리로 처리 - 절대경로
		// response.sendRedirect("/jsp/error/error_405.jsp");	
		//return;
			// sendError 는 브라우저의 url 주소가 바뀌지 않지만 sendRedirect는 url 주소가 바뀐다.
			
			// 방법 3. 쿼리스트링(GET방식)
		// 입력페이지(join_form.jsp)로 리다이렉트 이동
		// => 웹프로그램 요청시 필요한 값은 QueryString을 이용하여 전달
		// String message="비정상적인 방법으로 join_action.jsp 문서를 요청 하였습니다.";
		// URL 주소(QueryString 포함)는 영문자, 숫자, 일부 특수문자를 제외한 문자 사용 불가능
		// => 사용 불가능한 문자를 부호화 처리하면 URL 주소로 사용 가능
		// URLEncoder.encode(String s, String enc) : 문자열을 전달받아 원하는 캐릭터셋의
		// 부호화된 코드값의 문자열로 변환하여 반환하는 메소드
		// => 자바스크립트의 encodeURIComponent(s) 함수와 동일한 기능을 제공하는 메소드
			// GET 방식은 값을 전달할 때 부호화 처리하는 것을 권장
		...
		return;
			...
			// QueryString을 사용하는 경우도 많고 Session을 사용하는 경우도 많다. 프로그래머에게 달림.
	}
