...
<%
	...
	if(request.getMethod().equals("GET")) {//JSP 문서를 GET 방식으로 요청한 경우
		...
		response.sendRedirect("insertFormStudent.jsp");//입력페이지 이동 // 원래는 입력페이지보다는 error 페이지 or error 메세지를 보내는 것이 통상적인 방법 
		return;
	}
	...
	// 학생번호를 이용하여 STUDENT 테이블에 저장된 학생정보를 검색하여 반환하는 DAO 클래스의 메소드 호출
