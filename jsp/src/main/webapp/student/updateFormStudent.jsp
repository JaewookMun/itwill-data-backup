...
<%-- => [학생변경]을 클릭한 경우 학생정보 변경 처리페이지(updateStudent.jsp)로 이동 - 입력값(학생정보) --%>
...
<% 
	...
	// 학생번호를 전달하여 STUDENT 테이블에 저장된 해당 학생번호의 학생정보를 검색하여 반환하는 DAO 클래스의 메소드 호출
	...
	if(student==null){ // 검색된 학생정보가 없는 경우
		...
		// DB와 연동할 땐 DB에 전달되는 값들이 정상적인지 다 검사해주어야 한다.
...
<body>
	...
	<hr>
	<%-- 식별자(학생번호)는 사용자로부터 변경값을 입력받지 않고 처리페이지에 전달 --%>
	<%-- => 식별자는 출력하고 hidden 타입의 입력태그를 이용하여 식별자 전달 --%>
	<%-- => 입력태그에 식별자를 초기값으로 설정하고 readonly 태그속성 사용 --%>
		<%-- hidden / only 둘 중 무엇을 전달할 지 판단해야함. - hidden이 더 나아보임. --%>
		<%-- form 태그는 입력태그 값만 전달할 수 있다. --%>
