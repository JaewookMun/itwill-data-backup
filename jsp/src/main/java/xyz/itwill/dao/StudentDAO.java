...
// STUDENT 테이블의 학생정보에 대한 삽입,삭제,변경,검색 기능을 제공하는 클래스
public class StudentDAO extends JdbcDAO {
	...
	// 학생정보를 전달받아 STUDENT 테이블에 삽입하여 저장하고 삽입행의 갯수를 반환하는 메소드
	...
	// 학생번호를 전달받아 STUDENT 테이블에 저장된 학생정보를 검색하여 반환하는 메소드
	...
	public List<StudentDTO> selectAllStudentList(){
		...
		try {
			con=getConnection();
				...
				// 컬럼이 많을 경우 *를 사용하면 가독성이 느려진다. - 정석은 컬럼명만 표기
				// 다중행은 정렬 권장
